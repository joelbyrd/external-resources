...
var obj = { //non packslip-map data and static data first, 
	...
	returnToAddress: 	["«Vendor1»","«Vendor2»","«Vendor3»","«Vendor4»","«Vendor5»","«Vendor6»"], //or define your own, No commas between city/state
	...
	shipTo: 			["«ShipToFName» «ShipToLName»","«ShipToStreet»","«ShipToStreet2»","«ShipToRef1»","«ShipToRef2»","«ShipToCity» «ShipToState» «ShipToZip»"], // or use «ShipTo#»
	...
	uspsData: {
		companyName: null, // in this template, leave null and not "ChadCo"... library will now fail if this is null or chad-co instead of printing that out
		shipToZip: "«ShipToZip»",
		carrierSM: "«CarrierSM»", 
		construct: "«Construct»",
		shipDate: "«ShipDate»",
		shipFromZip: "«ShipFromZip»",  //shipfromzip will override shipFromArray zip  Used for setting usps Mail From 
		shipFromArray: ["«VendorName»","«VendorStreet»","«VendorStreet2»","«VendorRef1»","«VendorRef2»","«VendorCity» «VendorState», «VendorZip»"], //["«Vendor1»","«Vendor2»","«Vendor3»","«Vendor4»","«Vendor5»","«Vendor6»"], //Used for setting usps Mail From
...
var allLabelDefinitions = function () {
    ...
    this.CheckMissingUPSS = function () {
        ...
        console.warn("No check is performed for missing UPS Surepost data");
    }

    this.CheckMissingUSPS = function () {
        var labelProps = ["trackingNumber", "carrier"];
        var specificProps = ["construct", "companyName"];
        ...
        if (uspsObj.companyName && uspsObj.companyName.match(/chad\s*co/i)) {
            console.log("Check Prop: Company Name --> Cannot be ChadCo!")
            this.missingData.push("companyName-ChadCo!")
        }

    }

    this.CheckMissingUSPSS = function () {
        console.warn("No check is performed for missing USPS Surepost data");
    }
    ...
    this.loadDataFedex = function () {  //sets data based on serviceDesc
        ...
        if (this.labelInfo.returnToCountry) {
            this.labelInfo.returnToAddress.push(this.labelInfo.returnToCountry);
        }
        ...
        this.labelInfo.fedexData.logo = '<img src="' + this.labelInfo.assetLocation + logofile + '" class="logo" alt="FedEx Express">';
    ...
    this.loadDataUPSS = function () {
        ...
        // this is ie8 fix for date parsing
        console.log("making date out of " + this.labelInfo.upssData.urcDate + " removing '-' and replacing with '/': " + this.labelInfo.upssData.urcDate.replace(/\-/g, "/"));
        var urcDateObj = new Date(this.labelInfo.upssData.urcDate.replace(/\-/g, "/"));

        if (!isNaN(urcDateObj.getFullYear())) {
            urcM = urcDateObj.getMonth() + 1;
            urcY = urcDateObj.getFullYear();
        } else {
            console.log("Unable to parse date from URC: " + this.labelInfo.upssData.urcDate);
        }
        ...
        */
        ...
        for (var i = 0; i < newArr.length; ++i) {
            if (i == newArr.length - 1)
                newArr[i] = "<span style='font-size:14pt; font-weight:bold;'>" + newArr[i].replace(/[,]/g, '') + "</span>";
    ...
    this.loadDataUSPS = function () {
        ...
        switch (this.labelInfo.uspsData.carrierSM) {
            ...
            case "612":
                ...
                this.labelInfo.uspsData.serviceIcon = "<img src=" + this.labelInfo.assetLocation + "blank.gif' style='width:1.1in;height:1.1in;' alt='blank' />";
                this.labelInfo.weight = ConvertToOuncesOnly(this.labelInfo.weight);
                break;
        }//switch
    ...
    this.loadDataCanadaPost = function () {
        ...
        this.labelInfo.canadaPostData.logo = "<div class='divLogo'><img src='" + this.labelInfo.assetLocation + "canadapostlogoSM.png' alt='Logo'></div><div class='divLogoTopRightServiceIcon'>" + serviceType + "</div>";
    ...
    this.printLabelFedex = function () {
        ...
        if (this.missingData.length > 0) {
            ...
            if (isIE() == 8 && this.orientation !== "P") {
                cssClass = "fedexie8' style='width:7in;";
            ...
            outputBuffer += "&nbsp;<br />" +
                ...
                "<span style='font-size:10pt;font-weight:900;'><b>" + fedexInfo.shipToPhone + "</b></span>" +
    ...
    this.printLabelFedexNew = function () {
        ...
        if (this.missingData.length > 0) {
            ...
            var rotateBarcode = "";
            if (isIE() == 8 && this.orientation !== "P") {
                cssClass = "fedexie8 fedexnewie8 "+ (isPriority ? "priority" : "");
            }
            ...
            if (isIE() == 8) {
                fdxCityState = fdxAddressShipTo.splice(-1, fdxAddressShipTo.length - 1)[0].toUpperCase().replace(/[,]/g, "");
            }
            else {
                fdxCityState = fdxAddressShipTo.splice(-1)[0].toUpperCase().replace(/[,]/g, "");
            }
            ...
            var rturnCityStateCountry = rturnAddress.splice(idx).map(function (el) { return el.toUpperCase().replace(/[,]/g, "") });
            ...
            outputBuffer += ")" +
                ...
                "<span class='shiptophone' style='font-weight:900;'>" + fedexInfo.shipToPhone + "</span>";
    ...
    this.printLabelFedexSmartPost = function () {

        this.defaultErrorMessage = "Fedex Smartpost ";
        console.log(this.labelInfo);

        var fedexInfo = this.labelInfo.fedexData,
            valid = false,
            imgdata = fedexInfo.smartPostLabel || "",
            buffer = "<div class='inner'>"; // need this extra div due to the way rotations are done

        if (!imgdata || imgdata.indexOf("«") !== -1) {
            console.warn("SmartPost Label Info not provided");
            this.defaultErrorMessage += " label data was not provided.";
        } else {
            // make sure the string IS valid base64 
            try {
                atob(imgdata);
                valid = true;
            } catch (e) {
                console.warn("SmartPost Label data is not a valid base64 encoded string", e);
                this.defaultErrorMessage += " label data is not a valid base64 string.";
            }
        }

        if (valid) {
            var imgType = ['Express', 'Fedex', 'Freight', 'Ground', 'USPS'][fedexInfo.smartPostLabelType] || "";

            var imgTag = "<img class='smartpost-label-image' src='data:image/png;base64," + imgdata + "' ";
            if (imgType) {
                imgType += " Label";
                imgTag += " alt='" + imgType + "' title='" + imgType + "' ";
            }

            imgTag += "onerror='this.parentNode.innerHTML=\"There was an error displaying the SmartPost Label.\";' />";

            buffer += imgTag;


        } else {
            buffer += this.defaultErrorMessage + "<br/>";
        }
    ...
    this.printLabelUPS = function () {
        ...
        if (this.missingData.length > 0) {
            ...
            if (upsInfo.serviceIcon == "#")
                sOutputBuffer += "<img src='" + this.labelInfo.assetLocation + "UPSServiceIcon.gif' alt='G' />";
            else
    ...
    this.printLabelUSPS = function () {
        ...
        uspsInfo.shipToZip = formatUSZip(uspsInfo.shipToZip);

        if (uspsInfo.shipToZip == "fail")
            this.missingData.push("shipToZip");

        if (typeof uspsInfo.shipFromZip === "string" && uspsInfo.shipFromZip.indexOf("«") >= 0)
            uspsInfo.shipFromZip = "";
        ...
        if (this.missingData.length > 0) {
            ...
            outputBuffer += '' +
                ...
                '/barcode.ashx?Zip=' + uspsInfo.shipToZip +
    ...
    this.printLabelUPSS = function () {
        ...
        if (this.missingData.length > 0) {
            ...
            var billingtext, phoneNumberExt = "";
            var thirdParty;
            if (typeof upssInfo.isThirdParty === "undefined")
                thirdParty = this.labelInfo.upsData.isThirdParty; // fallback to UPS Data where it normally is
            else
                thirdParty = upssInfo.isThirdParty;
            ...
            // ie8 fix
            if (isNaN(bullDateObj)) {
                bullDateObj = (function (date) {
                    var parts = date.split("-");
                    var year = parts[0];
                    var month = parseInt(parts[1]) - 1;
                    var day = parts[2];
                    return new Date(year, month, day);
                })(this.labelInfo.upssData.orderEstDate);
            }

            if (isNaN(bullDateObj.valueOf())) {
                console.warn("Bullseye will not show: OrderEstDate: " + this.labelInfo.upssData.orderEstDate);
            }
    ...
    this.printLabelUSPSS = function () {
        ...
        if (this.missingData.length > 0) {
            ...
            var cssClass = "inner ups upsSure";
            if (isIE() == 8 && this.orientation !== "P") {
                cssClass = " upsie8 ' style='width:6in;";
            }
            ...
            /* the ordering here matters because of the way ie8 is rendering floated items there's an extra larger
             * margin to the top of the indica box that can't be removed even with css margins,etc. However if we move
             * it for anything thats NOT ie8 then it breaks the CSS for those items... easiest to just move it
             */
            var shipToAddressParts = compressAddressParts(shipToCaps);
            shipToAddressParts[shipToAddressParts.length - 1] = shipToAddressParts[shipToAddressParts.length - 1].replace(/[,]/g, '')

            if (isIE() == 8 && this.orientation !== "P") {
                sOutputBuffer +=
                    "<div class='box'>" +
                    "	<div class='wrap'>" +
                    "		<p class='indica'>" + surePostIndicia + "</p>" +
                    "	</div>" +
                    "</div>" +
                    "<div class='carrier'>Carrier - Leave If No Response</div>" +
                    "<div class='deliver'>" +
                    "	<b>USPS DELIVER TO:</b><br />" + shipToAddressParts.join("<br/>") +
                    "</div>";
            } else {
                sOutputBuffer +=
                    "<div class='carrier'>Carrier - Leave If No Response</div>" +
            ...
            // yes we mean to use Round=1 on this barcode. Its the only way we are getting anywhere close to the proper MLs
            sOutputBuffer +=
                "	    </div>" +
                "   </div>" +
                "	<div class='row7'>" +
                ...
                "       <div class='track'>" +
                "		    <div><img src='" + this.labelInfo.barcodeLocation + "/barcode.ashx?Zip=" + GetZipCode(this.labelInfo.shipTo)[0].substring(0, 5) + "&Track=" + upssInfo.surePostTrackingNo + "&Construct=C02&Type=USPS&X=3.45&Y=0.75&round=1' alt='Tracking Barcode' /></div>" +
		...
		*/
        ...
        if (label == 1) {
            ...
            buffer = String()
                ...
                + ' <img src="' + this.labelInfo.assetLocation + puroInfo.servIcon + '" /> '
                + '</div>'
                + '</div>'
                + '<div class="row2">'
                + '<div class="left">'
            ...
            buffer = String()
                ...
                + '<div style="height:.35in; width:32%; float:right; text-align:right;"><img src="' + this.labelInfo.assetLocation + 'Purolator_P.png" style="height:.35in;"></div>'
                + '</div>'
    ...
    this.printLabelMetapack = function () {
        buffer = String();
        if (this.labelInfo.metaPackData.consignment.indexOf("»") <= 0 && this.labelInfo.metaPackData.consignment.trim() != "") {
            document.write("" +
                "<div style='float:left;width:6in;height:4in;overflow:hidden;margin-top:0.2cm; margin-left:8pt;'> " +
                " <div> " +
                " <img src='/Printing/ShipLabl.ashx?Pozlookup=" + this.labelInfo.metaPackData.lookup + "&ExtType=2&Carrier=META&OrderType=4&Consignment=" + this.labelInfo.metaPackData.consignment + "' alt='' style='width:99%;'/> </div>" +
                "</div>");
        } else {
            document.write("" +
                "<div style='float:left;width:6in;height:4in;overflow:hidden;margin-top:0.2cm; margin-left:8pt;'> " +
                    "Error generating MetaPack label (consignment not found).<br />Please contact your StoreNet administrator." +
                "</div>");
        }
        document.write(buffer);
    }
...
function CarrierLabel(labelInfoIn) {
    ...
    this.orientation = (labelInfoIn.orientation || "P").toUpperCase();

    var theCountry = labelInfoIn.shipToCountry || "US";

    function isCanadaZip(zip) {
        return zip && /^\s*[ABCEGHJKLMNPRSTVXY]{1}\d{1}[A-Z]{1} *\d{1}[A-Z]{1}\d{1}\s*$/i.test(zip);
    }

    var imgRoot = (this.labelInfo.imgLocaton || "").replace(/[\/\\ ]+$/g, "");
    var assetRoot = (this.labelInfo.assetPath || "/shared/assets").replace(/(^[\/\\ ]+)|([\/\\ ]+$)/g, "")


    this.labelInfo.assetLocation = imgRoot + "/" + assetRoot + "/";

    var isCanada = /^\s*CA(N(ADA)?)?\s*$/i.test(theCountry);
    if (!isCanada && this.labelInfo.carrier == "UPSN") {
        //124 is the ISO code for Canada which is what we get for UPS data
        isCanada = this.labelInfo.upsData.shipToCountry == "124" || isCanadaZip(this.labelInfo.upsData.shipToZip);
    }
    ...
    function selectLabelType() {
        ...
        var outerclass;
        var isSurePost = (self.labelInfo.carrier == "UPSN" && (self.labelInfo.upsData.carrierSM == "SUREPOST" || self.labelInfo.carrierSM == "SUREPOST"));
        // need to distinguish this earlier in order to add sure post type CSS classes to document. 
        var surePostType = "";
        if (isSurePost && self.labelInfo.carrier == "UPSN") {
            if (self.labelInfo.upssData.trackingNumber.indexOf("YW") > -1
                ...
                surePostType = "ups"
                isCanada = isCanadaZip(self.labelInfo.upssData.surePostPOZip);
            }
            else {
                surePostType = "";
        ...
        if (self.labelInfo.orientation == "P") {
            ...
            if (isSurePost)
                ...
                outerclass = "landscapeLeft landscapeIE8";
            }
            else {
        ...
        if (isSurePost && surePostType) {
            if (surePostType == "usps") {
                outerclass += " surePostUSPS ";
            } else if (surePostType == "ups") {
                outerclass += " surePostUPS "
            }
        }
        ...
        if (self.labelInfo.carrier == "FXSP")
            outerclass += " img-only";
        ...
        if (self.labelInfo.carrier == "FXSP") {
            self.printLabelFedexSmartPost();
        } else if (self.labelInfo.carrier == "FDXN") {
            if (!newFedex) {
            ...
            if (surePostType == "usps") {
                ...
                // no label prints in this scenario
            }
        }
        ...
        else if (self.labelInfo.carrier == "META") {// METAPACK
            self.printLabelMetapack();
        }
...
// reduce: IE8+, FF3.0+, SF4+, Opera10.5+
// Production steps of ECMA-262, Edition 5, 15.4.4.21
// Reference: http://es5.github.io/#x15.4.4.21
// https://tc39.github.io/ecma262/#sec-array.prototype.reduce
// ie8 doesn't support calling defineProperty on the Array.prototype so just set it manually as a function
